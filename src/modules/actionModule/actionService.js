...
const createAction = async (
  { title, summary, views, categoryId, content, isFeatured },
  image,
  account
) => {
  try {
    const uploadImage = image
      ? await uploadSingleImageToCloudinary(image.path)
      : null
    if (image && !uploadImage)
      throw new ApiErr(StatusCodes.BAD_REQUEST, "Upload Image failed")
    ...
    if (actionExists)
      throw new ApiErr(
        ...
        "Action with this title already exists"
      )
    if (!cateExists)
      throw new ApiErr(StatusCodes.BAD_REQUEST, "Category ID does not exist")
...
const createActionDetail = async (data, account) => {
  ...
  if (!actionExists)
    throw new ApiErr(StatusCodes.BAD_REQUEST, "ActionId does not exist")
...
const getActionById = async (actionId) => {
  try {
    ...
    if (!actionDetail)
      throw new ApiErr(
        StatusCodes.NOT_FOUND,
        `ActionDetail not found with actionId: ${actionId}`
      )
    ...
    throw new ApiErr(
      StatusCodes.INTERNAL_SERVER_ERROR,
      `Error retrieving action: ${error.message}`
    )
  }
}
...
const updateAction = async (id, data, file, account) => {
  try {
    ...
    if (!updatedAction)
      throw new ApiErr(StatusCodes.NOT_FOUND, "Action not found")
    if (!updatedActionDetail)
      throw new ApiErr(StatusCodes.NOT_FOUND, "ActionDetail not found")

    return { updatedAction, updatedActionDetail }
  } catch (error) {
    throw new ApiErr(
      StatusCodes.INTERNAL_SERVER_ERROR,
      `Error updating action: ${error.message}`
    )
  }
}
...
const getTopViews = async () => {
  try {
    ...
    throw new ApiErr(
      StatusCodes.INTERNAL_SERVER_ERROR,
      `Error retrieving top views: ${error.message}`
    )
  }
}
...
const getFeatured = async () => {
  try {
    return await Actions.find({ isFeatured: true })
      .limit(5)
      .sort({ createdAt: -1 })
  } catch (error) {
    throw new ApiErr(
      StatusCodes.INTERNAL_SERVER_ERROR,
      `Error retrieving featured actions: ${error.message}`
    )
  }
}
...
const searchActions = async (searchTerm, page, limit, startDate, endDate) => {
  try {
    ...
    const actionDetails = await ActionDetail.find({
    ...
    const fullActionResults = actions.map((actionItem) => ({
      ...
      content:
        actionDetails.find(
          (detail) => detail.actionId.toString() === actionItem._id.toString()
        )?.content || null,
    }))
