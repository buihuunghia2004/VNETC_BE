...
const findAllNews = async (data) => {
  ...
  const [news, totalCount] = await Promise.all([
    News.find(query).skip(skip).limit(parseInt(limit)).sort({ createdAt: -1 }),
...
const createNews = async (
  ...
  try {
    ...
    if (!uploadImage)
      throw new ApiErr(StatusCodes.BAD_REQUEST, "Upload Image fail")
...
const getTopViews = async () => {
  try {
    const news = await News.find().limit(8).sort({ views: -1 })
    console.log(news)
    return news
  } catch (e) {
    throw e
...
const searchNews = async (searchTerm, page, limit, startDate, endDate) => {
  try {
    ...
    const newsDetails = await NewsDetail.find({
      newsId: { $in: newsIds },
    }).lean()
